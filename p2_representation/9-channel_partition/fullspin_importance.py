"""Full-spin importance / flip-targeting control for the channel-partition models,
REUSING exp-5 analyze() so the decisive random-flip control is identical to the
raw-clamp baseline (exp-5 model A: random-flip 0.97 vs actual flips 0.26).

The question (Matei-style): are the C->D spin flips concentrated on the full-spin
features that carry C's class info? The partition hypothesis is that confining the
label to group L makes C's class info less artificially imprinted on the I/N channels,
so the flip-targeting signature softens vs the baseline (which is exactly exp-5 model A
if our masking is a faithful no-op at baseline).

Only the architecture changes; the rollers are the STANDARD C/D rollers for every tag.
analyze() takes roll_C / roll_D as args, so this is fully drop-in.

Run (cluster): XLA_PYTHON_CLIENT_PREALLOCATE=false \
  ~/miniforge3/envs/darnax_hpc/bin/python p2_representation/9-channel_partition/fullspin_importance.py
Smoke:  python p2_representation/9-channel_partition/fullspin_importance.py --smoke
"""
...
def main():
    ...
    roll_C = fs5.make_roller(warmup, clamped, free)
    ...
    for tag in args.tags:
        ...
        for seed in args.seeds:
            ...
            per_seed.append(d)
            print(f"  [{tag:9s} s{seed}] pC->C={d['probe_C_on_C']:.3f} pC->D={d['probe_C_on_D']:.3f} "
    ...
    print("Reference: exp-5 model A pC->C=0.973 pC->D=0.261 rand_u=0.972 enrich=1.60.")

"""experiments4/fixedpoint_overlap_v3.py

Same as fixedpoint_overlap_v2.py but with SHORT dynamics (warmup=1, clamped=5,
free=5). Tests whether the 16×16 J1 geometry still saturates under shorter
iteration counts. Hyperparameters otherwise identical to Config 3.

Run on w01:
  ~/miniforge3/envs/darnax_hpc/bin/python experiments4/fixedpoint_overlap_v3.py
"""
...
def plot_saturation_curve(all_results, fig_path):
    ...
    for label, i, j in keys:
        ys = [2 * np.array(all_results[cp]["match"])[i, j] - 1 for cp in cps]
        ax.plot(cps, ys, "-o", label=label, alpha=0.85, markersize=5, lw=1.8)
    ...
    ax.set_title("Config 3 + SHORT dynamics (1, 5, 5):  saturation of the 16×16 matrix")
    ax.grid(alpha=0.3)
    fig.tight_layout()
...
def main():
    ...
    print("\nTraining + measuring (short dynamics 1-5-5)...", flush=True)
    for xb, yb in ds:
        x_hwc = to_hwc(xb)
    ...
    out_path = results_dir / "fixedpoint_overlap_v3.json"
    out_path.write_text(json.dumps(all_results, indent=2, default=float))
    print(f"Saved metrics to {out_path}")

    fig_path = figs_dir / "fixedpoint_overlap_v3_saturation.png"
    plot_saturation_curve(all_results, fig_path)
    print(f"Saved saturation curve to {fig_path}")

    print(f"\n{'=' * 78}")
    print("SUMMARY (Config 3 + SHORT dynamics 1-5-5)")
    print(f"{'=' * 78}")

"""experiments4/fixedpoint_overlap_matei_probe.py

Experiment 1 (16-point ABCD × ablations geometry) on Matei's probe-tuned
config (trial 13, probe_acc=0.468). Same structure as the Kassym version.

Dynamics: warmup=1 (default), clamped=4, free=13.
wback_type "conv1x1" maps to ChannelWBack (Matei confirmed they're equivalent).
The wback_strength key maps to the strength arg of ChannelWBack.

Run on w01:
  ~/miniforge3/envs/darnax_hpc/bin/python experiments4/fixedpoint_overlap_matei_probe.py
"""
...
# Matei probe-tuned config (trial 13).
# wback_type=conv1x1 is just Matei's old name for ChannelWBack;
# wback_strength maps to the strength arg.
CONFIG = {
    ...
    "strength_back":     0.8973904603571834,   # wback_strength
    "warmup_n_iter":     1,
    "clamped_n_iter":    4,
    "free_n_iter":       13,
}
...
def main():
    ...
    print("Building model (Matei probe-tuned config)...", flush=True)
    ...
    print("\nTraining + measuring (Matei probe, dynamics 1-4-13)...", flush=True)
    for xb, yb in ds:
        x_hwc = to_hwc(xb)
    ...
    out_path = results_dir / "fixedpoint_overlap_matei_probe.json"
    out_path.write_text(json.dumps(all_results, indent=2, default=float))
    print(f"\nSaved metrics to {out_path}")

    print(f"\n{'=' * 78}\nSUMMARY (Matei probe-tuned config)\n{'=' * 78}")

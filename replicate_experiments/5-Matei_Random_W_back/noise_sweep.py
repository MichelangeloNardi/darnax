"""5-Matei_Random_W_back/noise_sweep.py

Repeat the random W_back noise sweep (from 3-Random_W_back) but using
Matei's config (matei_W_out_cfg.json) instead of Kassym's best config.

Key difference: strength_back=0.168 (vs Kassym's 1.471), so the WBack
signal is ~9× weaker. Even σ=0.1 noise is already ~60% of the signal
amplitude, making the network potentially more sensitive to noise here.

clamped_n=11, free_n=14, warmup_n=1 (hardcoded as in matei_config_run.py).
cd=1.000 expected for all conditions (14 free steps fully converge).

noise_std values: [0.0, 0.1, 0.3, 0.5, 1.0]
  Relative to strength_back=0.168:
    0.0  = baseline
    0.1  ≈ 0.6× signal
    0.3  ≈ 1.8× signal
    0.5  ≈ 3.0× signal
    1.0  ≈ 6.0× signal
  (Compare: Kassym run had 0.3 ≈ 0.2× signal)

3 seeds × 10 epochs per condition + offline Wout fine-tune (10 ep).

Run on cluster (w02 or w03, NFS-shared with w01):
  ~/miniforge3/envs/darnax_hpc/bin/python \
    replicate_experiments/5-Matei_Random_W_back/noise_sweep.py
"""
...
def run_one_seed(noise_std, seed, cfg, ds, diag_rng, t_start, epoch_times_all) -> dict:
    ...
    # offline Wout fine-tune (Win+J1 frozen)
...
def run_one_noise(noise_std, cfg, ds, diag_rng, t_start, epoch_times_all) -> dict:
    ...
    print(f"  signal strength_back={cfg['strength_back']:.3f}"
          f"  noise/signal≈{noise_std/cfg['strength_back']:.2f}×", flush=True)
...
def plot_results(all_results, figures_dir, strength_back):
    ...
    xlabels = [f"σ={n:.1f}\n({n/strength_back:.1f}×)" for n in noises]
    ax.set_xticks(x); ax.set_xticklabels(xlabels, fontsize=9)
    ax.set_ylabel("Test accuracy")
    ax.set_title(
        f"Accuracy vs W_back noise std (Matei config, strength_back={strength_back:.3f})\n"
        f"epoch {EPOCHS}, mean±std over {n_seeds} seeds"
    )
    ...
    fig.suptitle(
        f"Training curves — W_back noise sweep, Matei config ({n_seeds} seeds ±1 std)\n"
        f"strength_back={strength_back:.3f}"
    )
    ...
    fig.suptitle(f"Weight norms — Matei config noise sweep (mean {n_seeds} seeds)")
    ...
    ax.set_title(f"J1 field contributions at epoch {EPOCHS} (mean {n_seeds} seeds, Matei config)")
    ...
    ax.set_title(f"Train/inference gap (C-D sim) — Matei config, {n_seeds} seeds")
...
def main():
    ...
    print(f"Noise values: {NOISE_VALUES}", flush=True)
    print(f"  strength_back={cfg['strength_back']:.3f}  "
          f"relative scales: {[round(n/cfg['strength_back'],2) for n in NOISE_VALUES]}×", flush=True)
    ...
    print(f"\n{'Noise':>7}  {'noise/signal':>13}  {'Head(final)':>12}  {'Probe(final)':>13}  {'Offline':>10}", flush=True)
    print("-" * 65, flush=True)
    for r in all_results:
        rel = r["noise_std"] / cfg["strength_back"]
    ...
    plot_results(all_results, figures_dir, cfg["strength_back"])

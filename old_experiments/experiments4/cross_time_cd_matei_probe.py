"""experiments4/cross_time_cd_matei_probe.py

Experiment 2 (per-image trajectory, unified baseline at batch 50) on Matei's
probe-tuned config (trial 13).
"""
...
CONFIG = {
    ...
    "strength_back":     0.8973904603571834,
    "warmup_n_iter":     1,
    "clamped_n_iter":    4,
    "free_n_iter":       13,
}
...
def main():
    ...
    print("\nBuilding model (Matei probe-tuned)...", flush=True)
    ...
    print("\nTraining (Matei probe-tuned)...", flush=True)
    t_train = time.time()
    batch_count = 0
    ...
    results = {
        "config": CONFIG,
        "config_label": "matei_probe (trial 13)",
    ...
    out_path = results_dir / "cross_time_cd_matei_probe.json"
    out_path.write_text(json.dumps(results, indent=2, default=float))
    print(f"\nSaved to {out_path}")

    print(f"\n{'=' * 60}\nSUMMARY (Matei probe-tuned, baseline {BASELINE_BATCH})\n{'=' * 60}")

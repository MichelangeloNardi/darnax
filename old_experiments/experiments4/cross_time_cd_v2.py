"""experiments4/cross_time_cd_v2.py

Same per-probe trajectory experiment as cross_time_cd.py, but with one change:
ALL 50 probes are baselined at the SAME training batch (BASELINE_BATCH = 50).
No probe is baselined at random init.

Why: random-init baselines artificially inflate the "drift" because they
compare the trained-network J1 state against a random-network J1 state, which
is a meaningless comparison (the random network never learned the probe).
With all baselines at batch 50, every per-probe stability curve starts at
1.0 by construction and any subsequent drift reflects real changes in the
trained network's representation of that image.

Run on w01:
  ~/miniforge3/envs/darnax_hpc/bin/python experiments4/cross_time_cd_v2.py
"""
...
N_EPOCHS = 2
MEASURE_EVERY = 50
BASELINE_BATCH = 50      # << ALL probes baselined here
PROBES_PER_CLASS = 5
N_CLASSES = 10
...
def main():
    ...
    assert max(first_seen) < BASELINE_BATCH, (
        f"BASELINE_BATCH ({BASELINE_BATCH}) must be after all probes' "
        f"first_seen (max {max(first_seen)})"
    )
    ...
    print(f"BASELINE_BATCH         : {BASELINE_BATCH}  (all probes baselined here)", flush=True)
    ...
    # per-probe stability lists (length matches len(measure_batches))
    ...
    # within-checkpoint overlaps (still useful)
    ...
    def record(batch_count):
        ...
        # within-checkpoint C↔D per image (dot product)
        cd_per_img = np.mean(np.sign(j1_C) * np.sign(j1_D),
                             axis=(1, 2, 3))   # (probes,)
        ...
        if batch_count == BASELINE_BATCH:
            ...
            # before baseline batch — nothing to compare against yet
            for pi in range(n_probes):
        ...
        if baseline_J1_C is not None:
            mean_stab_C = float(np.nanmean([sc[-1] for sc in stability_C_list]))
            mean_stab_D = float(np.nanmean([sd[-1] for sd in stability_D_list]))
            mean_cross  = float(np.nanmean([cc[-1] for cc in cross_C0_D_list]))
        else:
            mean_stab_C = mean_stab_D = mean_cross = float("nan")
        print(f"  batch {batch_count:5d}  "
              f"⟨stab_C⟩={mean_stab_C:.3f}  "
              f"⟨stab_D⟩={mean_stab_D:.3f}  "
              f"⟨C0↔D(t)⟩={mean_cross:.3f}  "
              f"⟨margin_D⟩={float(np.mean(margin_D_list[-1])):.3f}",
              flush=True)

    print("\nInitial measurement (batch 0, pre-training)...", flush=True)
    record(0)
    ...
    aligned_batches = (measure_batches_arr[None, :]
                       - BASELINE_BATCH)   # ALIGN to BASELINE_BATCH, not per-probe
    ...
    results = {
        ...
        "aligned_batches_vs_baseline": aligned_batches.tolist(),
    ...
    out_path = results_dir / "cross_time_cd_v2.json"
    out_path.write_text(json.dumps(results, indent=2, default=float))
    print(f"\nSaved to {out_path}")

    print(f"\n{'=' * 60}")
    print(f"SUMMARY (baseline at batch {BASELINE_BATCH})")
    print(f"{'=' * 60}")
    ...
    for ti, b in enumerate(measure_batches_arr):
        m_sc = float(np.nanmean(stab_C[:, ti]))
        m_cr = float(np.nanmean(cross[:, ti]))
        m_md = float(np.mean(margin[:, ti]))
        print(f"{int(b):>6}  {m_sc:>11.4f}  {m_cr:>10.4f}  {m_md:>13.4f}")
